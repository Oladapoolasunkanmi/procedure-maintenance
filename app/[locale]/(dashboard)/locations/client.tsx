...
import { locations, Location, Asset } from "@/lib/data"
        ...
        <div className="flex h-[calc(100vh-4rem)] flex-col gap-4 md:flex-row">
            ...
            <div className={cn("flex flex-col gap-4 transition-all duration-300", selectedId ? "hidden w-full md:flex md:w-1/3 lg:w-[450px]" : "w-full md:w-[800px]")}>
                ...
                <div className="flex items-center gap-2 overflow-x-auto pb-2 no-scrollbar">
                    <FilterButton label={t('filters.name')} icon={Box} value={filterConfig.name} onChange={(val) => setFilterConfig(prev => ({ ...prev, name: val }))} searchPlaceholder={t('searchPlaceholder')} />
                    <FilterButton label={t('filters.assets')} icon={Network} value={filterConfig.asset} onChange={(val) => setFilterConfig(prev => ({ ...prev, asset: val }))} searchPlaceholder={t('searchPlaceholder')} />
                    <FilterButton label={t('filters.teams')} icon={Users} value={filterConfig.team} onChange={(val) => setFilterConfig(prev => ({ ...prev, team: val }))} searchPlaceholder={t('searchPlaceholder')} />
                    <FilterButton label={t('filters.createdBy')} icon={Users} value={filterConfig.createdBy} onChange={(val) => setFilterConfig(prev => ({ ...prev, createdBy: val }))} searchPlaceholder={t('searchPlaceholder')} />
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="sm" className="h-auto p-0 text-muted-foreground font-bold hover:bg-transparent hover:text-muted-foreground/80 gap-1">
                                {sortConfig.key === 'createdAt'
                                    ? (sortConfig.direction === 'desc' ? t('sortBy.latest') : t('sortBy.oldest'))
                        ...
                        <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem onClick={() => setSortConfig({ key: 'createdAt', direction: 'desc' })}>
                                {t('sortBy.latest')} ({t('sortBy.label')})
                            </DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
                </div>
                    ...
                    <div className="flex flex-col divide-y">
                        {isLoading && locationsList.length === 0 ? (
                            <div className="flex flex-col items-center justify-center p-12 gap-4">
                                ...
                                <p className="text-muted-foreground animate-pulse">{t('empty.loading')}</p>
                            </div>
                        ) : (
                            <>
                                ...
                                {filteredLocations.length === 0 && (
                                    <div className="flex flex-col items-center justify-center p-12 text-center">
                                        ...
                                        <h3 className="text-lg font-semibold mb-1">{t('empty.noLocations')}</h3>
                                ...
                                {nextBlockId && (
                                    <div className="p-4 text-center">
                                        <Button
                                            ...
                                            disabled={isLoading}
                                        >
                                            {isLoading ? t('empty.loading') : t('assets.loadMore')}
                                        </Button>
                                    </div>
                                )}
                            </>
...
function FilterButton({ label, icon: Icon, value, onChange, searchPlaceholder }: { label: string, icon: any, value: string, onChange: (val: string) => void, searchPlaceholder?: string }) {
                    ...
                    <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
                        <DialogContent>
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                    {t.rich('detail.deleteDialog.description', {
                                        name: location.name,
                            ...
                            <DialogFooter>
                                <Button variant="outline" onClick={() => setShowDeleteDialog(false)}>{t('detail.deleteDialog.cancel')}</Button>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden min-h-0 mt-6">
                ...
                <div className="flex-1 overflow-y-auto min-h-0">
                    <TabsContent value="details" className="m-0 p-6 space-y-8">
                        ...
                        {/* Description Section */}
                        <div className="space-y-2">
                            <h3 className="font-semibold">{t('detail.sections.description')}</h3>
                            <p className="text-sm text-muted-foreground">
                                {location.description || t('detail.noDescription')}
                            </p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-1">
                                <h3 className="font-semibold">{t('detail.sections.address')}</h3>
                                <p className="text-sm text-muted-foreground">{location.address || "-"}</p>
                            </div>

                            <div className="space-y-1">
                                <h3 className="font-semibold">{t('detail.sections.teams')}</h3>
                                    ...
                                    {location.teamsInCharge && location.teamsInCharge.length > 0 ? (
                                        location.teamsInCharge.map(teamId => {
                                            ...
                                            return team ? (
                                                ...
                                                <Badge key={teamId} variant="outline" className="text-muted-foreground">{t('detail.unknownTeam')} ({teamId})</Badge>
                                            )
                                        })
                                    ) : (
                            ...
                            <div className="space-y-1">
                                <h3 className="font-semibold">{t('detail.sections.barcode')}</h3>
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between">
                                <h3 className="font-semibold flex items-center gap-2">
                                    {t('detail.sections.subLocations')} ({subLocationsCount})
                                </h3>
                                <Button
                                    variant="ghost"
                                    size="sm"
                        ...
                        <div className="space-y-4">
                            <h3 className="font-semibold flex items-center gap-2">
                                ...
                                {t('detail.sections.assets')} ({assets.length})
                            </h3>
                            {assets.length > 0 ? (
                                <div className="space-y-2">
                                    {assets.map(asset => (
